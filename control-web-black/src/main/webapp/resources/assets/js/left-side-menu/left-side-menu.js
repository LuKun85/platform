$(document).ready(function() { 
	...
	// toggle skin select	
	$("#left-side-menu #toggle").click(function() {

		if($(this).hasClass('active')) {
			$(this).removeClass('active')
			$('#left-side-menu').animate({ left:0 }, 100);
			$('.wrap-fluid').css({"width":"auto","margin-left":"250px"});
			$('.navbar').css({"margin-left":"240px"});

			$('#left-side-menu li').css({"text-align":"left"});
			$('#left-side-menu li span, ul.topnav h4, .side-dash, .noft-blue, .noft-purple-number, .noft-blue-number, .title-menu-left').css({"display":"inline-block", "float":"none"});
			...
			//$('#left-side-menu').animate({ left:-200 }, 100);
			$('#left-side-menu').animate({ left:-200 }, 100);

			$('.wrap-fluid').css({"width":"auto", "margin-left":"50px"});
			$('.navbar').css({"margin-left":"50px"});

			$('#left-side-menu li').css({"text-align":"right"});
			$('#left-side-menu li span, ul.topnav h4, .side-dash, .noft-blue, .noft-purple-number, .noft-blue-number, .title-menu-left').css({"display":"none"});
	...
	setTimeout(function(){ $("#left-side-menu #toggle").addClass('active').trigger('click'); },10)
